...
- **Elon Musk (2023)**: The Algorithm = question → delete → simplify → accelerate → automate (in that order; most engineers start at step 5); Idiot Index = part cost ÷ raw material cost (high ratio = stupid manufacturing); "the factory is the product"; Demon Mode (dark psychological episodes from childhood trauma) is inseparable from the drive that built SpaceX and Tesla
- **Isaacson's Twitter critique**: Twitter acquisition is Musk's only major bet that produced clear net harm; unlike SpaceX/Tesla (engineering problems with physics constraints), Twitter is a social/political problem — The Algorithm doesn't apply; no first principles; no idiot index; no clear failure criteria
- **Key contrast — Jobs vs. Musk**: both had brutal management and binary thinking; Jobs had more human warmth and a coherent aesthetic sense; Musk's emotional coldness appears deeper and more structural; Jobs's demons were episodic, Musk's are systemic
